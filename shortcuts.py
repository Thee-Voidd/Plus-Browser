...
def register_shortcuts(window):
    ...
    # Bookmarks & History Windows
    QShortcut(QKeySequence("Ctrl+D"), window, activated=window.bookmark_current_page)
    QShortcut(QKeySequence("Ctrl+B"), window, activated=window.open_bookmarks_dialog)
    QShortcut(QKeySequence("Ctrl+H"), window, activated=window.open_history)